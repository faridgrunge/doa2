  ...
  if (typeof hap_group !== "undefined") {
    if (hap_group == "wrap_multi" || hap_group == "wrap_multi2") {
      ...
      var track,
        ...
        thumb,
        download,
        _item = jQuery(instance).closest(".playlistItem");
      ...
      if (type && mp3) {
        ...
        if (_item.attr("data-title") != undefined)
          title = _item.attr("data-title");
        if (_item.attr("data-thumb") != undefined)
          thumb = _item.attr("data-thumb");
        if (_item.attr("data-download") != undefined)
          download = _item.attr("data-download");
...
function audioPlayerSoundPlay(instance, sound_id, counter) {
  ...
  if (
    typeof hap_group !== "undefined" &&
    typeof hap_players !== "undefined" &&
    hap_players.length &&
    typeof soundArr !== "undefined" &&
    soundArr.length
  ) {
    var i = 0,
      len = soundArr.length;
    for (i; i < len; i++) {
      if (sound_id != soundArr[i].sound_id) {
        ...
        if (typeof api_checkAudio !== "undefined")
          api_checkAudio(soundArr[i].player_id, "pause");
      }
    }
  }
}
...
function playlistItemDisabled(instance, sound_id, target, id) {
  ...
  if (sound_id == "wall") {
    ...
    if (title) {
      ...
      p.appendTo(jQuery(target)).css(
        "marginTop",
        -p.outerHeight(true) / 2 + "px"
      );
    }
  }
}
...
function playlistEmpty(instance, sound_id) {
  ...
  if (sound_id == "drag_text2" || sound_id == "drag_thumb2") {
    ...
    if (instance.find("p[class=drag_info]").length == 0) {
      instance
        .find(".playlist_inner")
        .prepend($('<p class="drag_info">DRAG THE SONGS IN HERE!</p>'));
...
var trackList2_local1 = [
  ...
  },
  {
    type: "local",
    mp3:
      "../media/audio/1/Tim_McMorris_-_Give_Our_Dreams_Their_Wings_To_Fly.mp3",
    ogg:
      "../media/audio/1/Tim_McMorris_-_Give_Our_Dreams_Their_Wings_To_Fly.ogg",
...
var trackList2_ofm_playlist = [{ type: "ofm_playlist", path: "1rp7" }];

var trackList2_ofm_project = [{ type: "ofm_project", path: "edB6" }];
...
var isMobile = /Android|webOS|iPhone|iPad|iPod|sony|BlackBerry/i.test(
  navigator.userAgent
);
...
function checkFlash(dataArr, embed_circle) {
  ...
  if (!html5Support) {
    ...
    for (i; i < len; i++) {
      ...
      ).appendTo(flashAudioWrapper);

      var flashMainWrapper = jQuery("<div/>").addClass("flashMain");
      var flashMain = jQuery(
        "<div id='" +
          f_id +
          "'><a href='http://www.adobe.com/go/getflashplayer'><img src='http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif' alt='Get Adobe Flash player' /></a></div>"
      ).appendTo(flashMainWrapper);
      ...
      if (embed_circle) {
        ...
        var flashCircle = jQuery(
          "<div id='" +
            c_id +
            "'><a href='http://www.adobe.com/go/getflashplayer'><img src='http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif' alt='Get Adobe Flash player'/></a></div>"
        ).appendTo(flashCircleWrapper);
        dataArr[i].holder
          .find("div[class=circlePlayer]")
          .append(flashCircleWrapper);
      }
    }
  } else {
    i = 0;
    for (i; i < len; i++) {
      if (dataArr[i].settings.useOnlyMp3Format && !mp3Support) {
        //use flash to play mp3 on browsers that do not support mp3
...
function flashYoutubeData(data) {
  if (hap_players[data.id])
    hap_players[data.id].flashYoutubeData(data.bl, data.bt, data.t, data.d);
}
...
function flashAudioData(data) {
  if (hap_players[data.id])
    hap_players[data.id].flashAudioData(data.bl, data.bt, data.t, data.d);
}
...
function flashCircleOverLoader(data) {
  if (hap_players[data.id])
    hap_players[data.id].flashCircleOverLoader(data.val);
}
...
function notify_popup() {
  //called from popup window when popup window has opened!
  //console.log('notify_popup');
  if (hap_popup && hap_popup.initPopup != undefined) {
    //dont do anything if we are not going to be able to open popup!
...
function open_player(player) {
  ...
  hap_players[0] = jQuery("#componentWrapper")
    .css("display", "block")
    .html5audio(hap_settings); //show player before init
  if (!html5Support) hap_players[0].embedFlash();
  hap_players[0].find(".popup_open").css("display", "block");
}

function open_popup(url, w, h, resizable) {
  //public api to open popup
  ...
  var cw = (window.screen.width - w) / 2,
    ch = (window.screen.height - h) / 2; //center popup in window

  //if popup window not already opened!
  if (!hap_popup || hap_popup.closed) {
    if (resizable) {
      hap_popup = window.open(
        url,
        "hap_popup_window",
      ...
      hap_popup = window.open(
        url,
        "hap_popup_window",
        "menubar=no,toolbar=no,location=no,width=" +
          w +
          ",height=" +
          h +
          ",left=" +
    ...
    if (!hap_popup) {
      alert(
        "Music Player can not be opened in a popup window because your browser is blocking Pop-Ups. Please allow Pop-Ups in browser for this site to use the Music Player."
      );
      return false;
    }
    if (window.focus) {
...
function updatePlayerData(isPopup) {
  ...
  if (!isNaN(hap_players[0].getVolume()))
    hap_settings.defaultVolume = hap_players[0].getVolume(); //volume
  if (!isNaN(hap_players[0].getActiveItem()))
    hap_settings.activeItem = hap_players[0].getActiveItem(); //active item
  ...
  if (playlistLoaded) {
    ...
    if (!hidden_playlist) {
      ...
      if (playlist.length > 0) {
        ...
        if (isPopup && isIE) {
          var curr_playlist = jQuery(
            playlist.clone(true, true).wrap("<p>").parent().html()
          ); //used for popup in IE (HIERARCHY_REQUEST_ERROR)!! important!
        } else {
          var curr_playlist = playlist.clone(true, true);
        }
        jQuery(hap_settings.playlistList)
          .find("#" + activePlaylist)
          .remove(); //remove playlist with active id from playlist_list, we are going to paste new version inside (with the same id attribute) in case playlist content has been changed (added/removed tracks/order etc...)
      ...
      if (playlist.length > 0) {
        ...
        if (isPopup && isIE) {
          jQuery(
            playlist.clone(true, true).wrap("<p>").parent().html()
          ).appendTo(curr_playlist); //used for popup in IE (HIERARCHY_REQUEST_ERROR)!! important!
        } else {
    ...
    mediaPlaying
      ? (hap_settings.autoPlay = true)
      : (hap_settings.autoPlay = false);
